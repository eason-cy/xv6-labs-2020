...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  kama_kvmcopymappints(p->pagetable,p->kama_kernelpgtbl,0,p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    //内核页表中的映射同步扩大
    if(kama_kvmcopymappints(p->pagetable,p->kama_kernelpgtbl,sz,n) != 0) {
...
fork(void)
{
  ...
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0 || kama_kvmcopymappints(np->pagetable,np->kama_kernelpgtbl,0,p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
