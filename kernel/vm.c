...
void
kama_kvm_map_pagetable(pagetable_t pagtbl)
{
  // uart registers
  kvmmap(pagtbl,UART0, UART0, PGSIZE, PTE_R | PTE_W);

  // virtio mmio disk interface
  kvmmap(pagtbl,VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);

  // CLINT
  // kvmmap(pagtbl,CLINT, CLINT, 0x10000, PTE_R | PTE_W);
  // CLINT
  // PLIC
  kvmmap(pagtbl,PLIC, PLIC, 0x400000, PTE_R | PTE_W);

  // map kernel text executable and read-only.
  kvmmap(pagtbl,KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  kvmmap(pagtbl,(uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);

  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  kvmmap(pagtbl,TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
}
void
...
int kama_kvmcopymappints(pagetable_t src, pagetable_t dst , uint64 start, uint64 sz) {
  pte_t* pte;
  uint64 pa,i;
  uint flags;
